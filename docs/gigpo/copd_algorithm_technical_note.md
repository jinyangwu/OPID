...
本文档说明当前仓库中 COPD 算法的实现形态、数学目标和训练流程。当前
COPD 是一种面向多步智能体任务的 critic-free on-policy 优化方法。它以
GRPO 风格的组内 episode-relative advantage 为基础，并引入由大模型轨迹
分析产生的 hindsight guidance。需要强调的是，分析大模型不直接给 reward
或 value；它只把一条完整轨迹总结为 `episode_hint` 和若干 `step_hints`。
训练器随后将这些 hint 注入当前 observation，并用当前策略重新计算“原始
动作”在增强 prompt 下的 log-prob。增强 prompt 与原始 prompt 的
log-prob 差值构成 token-level teacher advantage。

在当前打开的 AlfWorld 启动脚本
[`examples/copd_trainer/run_alfworld_copd_guide.sh`](../../examples/copd_trainer/run_alfworld_copd_guide.sh)
中，实际运行的是 episode-level COPD：

- `algorithm.adv_estimator=copd`
- `algorithm.copd.mode=mean_norm`
- `algorithm.copd.step_advantage_w=0.0`
- `algorithm.copd.episode_hint_teacher_advantage_w=0.001`
- `algorithm.copd.step_hint_teacher_advantage_w=0.001`
...
COPD 的目标是在稀疏、延迟且高方差的 agent reward 场景下，保留
...
flowchart TD
    ...
    E --> F[episode_hint and step_hints]
...
生成 hindsight guidance；之后用增强 observation 重新打分原始 response，
...
| \(\ell^{ep}\) | episode hint 增强 prompt 下的 token log-prob | `episode_teacher_log_prob` |
| \(\ell^{step}\) | step hint 增强 prompt 下的 token log-prob | `step_teacher_log_prob` |
...
对于 COPD 和 GiGPO，训练器还会计算 discounted step return：
...
任务组 \(g(i)\)。COPD 首先计算组内相对 episode advantage。
...
这一部分由
[`gigpo/core_gigpo.py`](../../gigpo/core_gigpo.py) 中的
`episode_norm_reward` 和 `compute_copd_advantage_components` 实现。

## 6. 可选的 state-relative step advantage

COPD 可以复用 GiGPO 的 state grouping 机制。当
`algorithm.copd.step_advantage_w` 非零时，训练器会在同一个 `uid` 内按照
...
不过在当前 AlfWorld COPD 脚本中：

$$
w_{step}=0.
$$

因此该项不会进入最终 advantage。普通 episode-level COPD 的配置校验也要求
`step_advantage_w=0.0`。如果需要保留 GiGPO step advantage，应使用 SOPD
路径，即设置 `algorithm.copd.singleton_only=True`。

## 7. Hindsight guidance 生成

COPD 会把每条 `traj_uid` 对应的 step 重新组装为有序轨迹记录，每个 step
包含：
...
轨迹分析由
[`gigpo/copd.py`](../../gigpo/copd.py) 中的 `COPDEpisodeAnalyzer`
...
- 若 episode 成功，`episode_hint` 应抽象为可复用 workflow；
- 若 episode 失败，`episode_hint` 应抽象为 avoidance rule；
- `step_hints` 是面向策略的短 imperative guidance，用于少量关键步。

当前 AlfWorld 脚本设置
`algorithm.copd.analysis_max_step_hints_per_traj=5`，所以每条轨迹最多保留
5 个 step-level hints。若 LLM 输出 JSON 解析失败，分析器会重试；若最终仍
失败或缺少必需字段，该轨迹不会产生 teacher signal。

## 8. Observation 增强与 teacher scoring

对每条成功分析的轨迹，普通 COPD 会将该轨迹中的所有 step 视为可进行
hindsight teacher scoring 的样本。具体使用哪一种 guidance 由如下规则决定：

1. 如果当前 step 有非空 `step_hint`，且
   `step_hint_teacher_advantage_w > 0`，则使用 step-level guidance；
2. 否则，如果 episode-level teacher 权重大于 0，则使用 `episode_hint`；
3. 若两类权重均为 0，则该 step 不产生 teacher log-prob。

guidance 注入逻辑位于
[`agent_system/memory/episode_hint.py`](../../agent_system/memory/episode_hint.py)。
增强 observation 的文本形态为：

```text
Episode-Level Guidance
Refer to this episode-level guidance when deciding what action to take in the current episode:
[...]
...
令 \(h^{ep}(o_i)\) 表示插入 episode hint 后的 observation，
\(h^{step}(o_i)\) 表示插入 step hint 后的 observation。训练器不会重新采样
...
prompt 下的条件概率”。这使 COPD 更接近一种 prompt-conditioned
self-distillation。
...
episode hint 或 step hint 进行 teacher scoring。若 \(\Delta>0\)，表示
hindsight guidance 使原始动作更可能，PPO 更新会更倾向于强化该动作；若
\(\Delta<0\)，则表示 guidance 认为该动作在增强上下文下更不合理，对应产生
抑制作用。
...
- `algorithm.copd.normalize_teacher_adv`
- `algorithm.copd.clip_teacher_adv`

当前 AlfWorld 脚本设置 `normalize_teacher_adv=False`，且未启用
`clip_teacher_adv`。

## 10. 最终 COPD advantage

COPD 的最终 token-level advantage 为：
...
+
w_{stepHint} \Delta^{step}_{i,\ell}.
$$
...
该融合逻辑位于
[`gigpo/core_gigpo.py`](../../gigpo/core_gigpo.py) 中的
`compute_copd_outcome_advantage` 和 `compute_copd_advantage_components`。

## 11. PPO 目标

得到 \(A^{COPD}\) 后，训练器使用标准 PPO actor update。令：
...
\min
\left(
\rho_{i,\ell}(\theta) A^{COPD}_{i,\ell},
\operatorname{clip}
...
```text
Algorithm: One COPD training step
...
Input:
  ...
  environment E
  discount gamma
  weights w_step, w_ep, w_stepHint
...
3. Run COPD analysis if enabled.
   Reconstruct each traj_uid into ordered step records.
   Send the trajectory to COPDEpisodeAnalyzer.
   Parse episode_summary, episode_hint, step_hints.
   Drop trajectories with invalid JSON or missing required hints.

4. Build augmented observations.
   For each analyzed step:
     if step_hint exists and w_stepHint > 0:
       insert Critical-Step Guidance
       mark q_step = 1
     else if episode_hint is enabled:
       insert Episode-Level Guidance
       mark q_ep = 1
...
| `algorithm.copd.analysis_num_workers` | `128` | 并发分析轨迹 |
...
| COPD 训练器集成 | [`verl/trainer/ppo/ray_trainer.py`](../../verl/trainer/ppo/ray_trainer.py) |
| LLM 轨迹分析器 | [`gigpo/copd.py`](../../gigpo/copd.py) |
| hint 注入 observation | [`agent_system/memory/episode_hint.py`](../../agent_system/memory/episode_hint.py) |
...
| COPD 默认配置 | [`verl/trainer/config/ppo_trainer.yaml`](../../verl/trainer/config/ppo_trainer.yaml) |
| AlfWorld COPD 启动脚本 | [`examples/copd_trainer/run_alfworld_copd_guide.sh`](../../examples/copd_trainer/run_alfworld_copd_guide.sh) |

## 15. 与 SOPD 的关系

SOPD 即 Singleton OPD。它复用 COPD 的 LLM guidance 与 teacher scoring
管线，但只对 GiGPO state group 中 size 为 1 的 singleton steps 应用 OPD
teacher signal。SOPD 的目的不是替代 GiGPO step advantage，而是在 state
group 没有可比较样本时，为这些孤立状态补充 hindsight teacher signal。
...
$$
w_{step}>0,\quad \text{singleton\_only=True}.
$$

相关说明见
[`examples/sopd_trainer/README.md`](../../examples/sopd_trainer/README.md)。

## 16. 讨论与局限

1. COPD 的 teacher signal 是 prompt-conditioned self-distillation，不是
   外部 reward model。它度量的是 hindsight guidance 对当前策略原始动作
   log-prob 的影响。

2. 当前普通 COPD 路径要求 `selector=llm`，且要求
   `algorithm.copd.step_advantage_w=0.0`。若要使用 state-relative step
   advantage，应转向 SOPD 配置。

3. 当前 teacher scoring 只支持 text prompt。若 batch 中存在
   `multi_modal_inputs`，COPD teacher signal 会被跳过。
...
LLM 负责把完整轨迹转化为可执行的 hindsight guidance；当前策略负责将该
guidance 转换为 token-level log-prob delta。最终，episode outcome signal
与 teacher prompt signal 共同形成用于 PPO 更新的 \(A^{COPD}\)。
