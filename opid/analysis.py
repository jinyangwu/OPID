...
class OPIDEpisodeAnalyzer:
    ...
    def _analyze_episode_with_openai(
        ...
        logger.warning(
            "OPID analyzer failed to produce valid JSON after %s attempt(s); skipping teacher skill for this trajectory: %s",
            parse_attempts,
            last_error,
        )
        return {
    ...
    def _get_parse_attempts(self) -> int:
        raw_attempts = os.environ.get("OPID_ANALYSIS_PARSE_RETRIES", "2")
        try:
            return max(1, int(raw_attempts))
        except (TypeError, ValueError):
            logger.warning("Invalid OPID_ANALYSIS_PARSE_RETRIES=%r; using 2.", raw_attempts)
            return 2
...
"""
    ...
    def _extract_json_object(self, response: str) -> Dict[str, object]:
        ...
        if not saw_object_start:
            raise ValueError("No JSON object found in OPID analyzer response.")
        raise ValueError(f"Could not parse JSON object in OPID analyzer response: {last_error}") from last_error
