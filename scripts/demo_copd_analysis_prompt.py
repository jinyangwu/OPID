#!/usr/bin/env python3
"""Rebuild a COPD episode-analysis prompt from a saved analysis jsonl row."""
...
def import_copd_episode_analyzer():
    try:
        from gigpo.copd import COPDEpisodeAnalyzer

        return COPDEpisodeAnalyzer
    except ModuleNotFoundError as exc:
        if exc.name != "torch":
            raise

        # This demo only needs the prompt-construction methods in gigpo.copd.
        ...
        sys.modules.pop("gigpo.copd", None)
...
DEFAULT_JSONL = (
    "/raid3/data/GTPO/MODELS/ckpt/"
    "copd-grpo_qwen2.5_1.5b_webshop_llm-5_episode-step-hint-plus-v2_opd-adv-0.001_exp1/"
    "copd_analysis/step_00000001.jsonl"
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Load one row from a saved COPD analysis jsonl file, reconstruct the episode steps, "
            "call the current COPDEpisodeAnalyzer._build_episode_analysis_prompt, and optionally call the LLM."
        )
    )
    parser.add_argument("--jsonl", default=DEFAULT_JSONL, help="Saved COPD analysis jsonl file.")
    ...
    parser.add_argument("--backend", default="openai", help="COPD analysis backend. Usually openai.")
    parser.add_argument("--analysis-mode", default=None, help="Override analysis mode from the saved row.")
    parser.add_argument("--max-step-hints-per-traj", type=int, default=5)
    ...
    *,
    ...
    payload = {
        ...
        "config": {
            ...
            "max_step_hints_per_traj": args.max_step_hints_per_traj,
...
def main() -> int:
    ...
    analyzer_cls = import_copd_episode_analyzer()
    ...
    print("Loaded COPD analysis row:")
