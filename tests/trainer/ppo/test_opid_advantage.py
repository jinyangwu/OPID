...
from gigpo.core_gigpo import compute_opid_advantage_components


def test_opid_uses_independent_episode_and_step_skill_teacher_weights():
    response_mask = torch.ones((2, 2))
    old_log_prob = torch.zeros((2, 2))

    _, _, teacher_advantages, scores, metrics = compute_opid_advantage_components(
    ...
    assert metrics["opid/adv/episode_skill_teacher_weight"] == 0.25
    assert metrics["opid/adv/step_skill_teacher_weight"] == 0.5


def test_opid_adds_episode_and_step_skill_teacher_advantages_on_same_step():
    response_mask = torch.ones((1, 2))
    old_log_prob = torch.zeros((1, 2))

    _, _, teacher_advantages, scores, _ = compute_opid_advantage_components(
